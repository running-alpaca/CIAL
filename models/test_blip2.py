...
def format_question(question, planning=False):
    ...
    # return f"Please answer the following question in a detailed and helpful way. List steps to follow if needed. Please don't answer with only one word!. Question: {question} Answer:"
...
class TestBlip2:
    ...
    @torch.no_grad()
    def generate(self, image, question, max_new_tokens=300, planning=False):
        image = get_image(image)
        image = self.vis_processors["eval"](image).unsqueeze(0).to(self.device, dtype=self.dtype)
        prompt= format_question(question, planning=planning)
        print("*prompt*:",prompt)
