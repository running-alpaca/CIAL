...
def prompt_template_caption_ret(question, caption, ret_caption,planning=False):
    # return f"Question: {question}\nShort answer:"
    if ret_caption=='':
        if planning:
            return f'Answer the following question in a detailed and helpful way.\nHere is the caption of the image: {caption} \nQuestion: {question}\nAnswer: '
...
class TestPandaGPT:
    def __init__(self, size='7b', device=None):
        if size == '7b':
            vicuna_path = "./VLMs/vicuna-7b-v1-1"
            delta_path = "./VLMs/pandagpt/pandagpt_7b_max_len_1024/pytorch_model.pt" 
        else:
            vicuna_path = "./VLMs/vicuna-13b-v1-1"
            delta_path = "./VLMs/pandagpt/pandagpt_13b_max_len_400/pytorch_model.pt"
        args = {
            'model': 'openllama_peft',
            'imagebind_ckpt_path': "./VLMs/pandagpt/imagebind_ckpt",
    ...
    def generate(self, image, question, caption,ret_img, ret_caption, max_new_tokens=256, planning=False):
        ...
        response = self.model.generate({
            'prompt': prompt_template_caption_ret(question, caption,ret_caption,planning=planning),
